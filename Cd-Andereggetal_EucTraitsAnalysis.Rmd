...
model <-  lmer(LMA~pMDscaled +  (1|Plottag) + (1|Treetag), data = traits.b, subset=Species=="OVAT"& pPPT>0, REML=F)
climvar<- "MD"
dataz <- traits.b[which(traits.b$Species=="OVAT" & traits.b$pPPT>0 & traits.b$LMA>0),]
  ref.group <- ranef(model)
  ref.var.group1 <- tapply(residuals(model, type="pearson", level=1),
                           dataz[,names(ref.group[1])], var)
  ref.var.group2 <- tapply(residuals(model, type="pearson", level=1),
...
plot(sqrt(ref.group[[1]]$var)~ref.group[[1]][,paste0("p",climvar)], ylab="SD of LMA (g/cm2)", col="grey", xlab="Moisture Deficit (mm)")
